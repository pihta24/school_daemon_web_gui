...
import {Corpus, privateKey} from "@/database";
...
export function create_msg(command: string, hostname: string, other_data: string, key: string): Buffer {
    ...
    return add_hostname(msg, hostname);
}
...
export function send_msg(msg: Buffer, corp: Corpus) {
    ...
    const ips_iter = ips[Symbol.iterator]();
    const send = () => {
        const ip = ips_iter.next().value;
        if (!ip) return;
        let m = add_rand_id(sign(add_timestamp(msg), privateKey));
        client.send(m, 55555, ip);
        setTimeout(send, 10);
    }
    send();
}